...
bool makeEthAccount(const string& private_key, const string& ethpswd)
{
	...
	string strRes;
	strCmd = "rm /usr/cess-proof-parameters/priv.txt";
...
bool makeShellCmd(const string& minerid, const string& keystore, const string& ethpswd, const string& ip, const string& disk, const string& spaceTb)
{
	...
	string strCmd = "./cessnode -k /root/.ethereum/keystore/mykey -p " + ethpswd + " -m ";
	strCmd += disk;
	strCmd += " -n ";
	strCmd += minerid;
	strCmd += " -s ";
...
bool deployCess()
{
	...
	string strMiner, strPriv, strPswd, strIp, strDisk, strSpace;
	if (!getParams(strMiner, strPriv, strPswd, strIp, strDisk, strSpace))
		return false;

	if (!makeEthAccount(strPriv, strPswd))
		return false;
	...
	if (!makeShellCmd(strMiner, strKey, strPswd, strIp, strDisk, strSpace)) {
		return false;
	}
